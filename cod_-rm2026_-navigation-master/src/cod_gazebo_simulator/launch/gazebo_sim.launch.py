...
def setup_launch(context, *args, **kwargs):
    ...
    # M2修复: 地图根据 world 参数选择
    map_yaml = os.path.join(pkg_sim, "maps", f"{world}.yaml")
    if not os.path.exists(map_yaml):
        map_yaml = os.path.join(pkg_sim, "maps", "rmul2026.yaml")

    # 6. Map server + AMCL (C3修复: 添加定位)
    map_server = TimerAction(period=9.0, actions=[
        Node(package="nav2_map_server", executable="map_server",
             parameters=[{"yaml_filename": map_yaml, "use_sim_time": True}]),
        Node(package="nav2_lifecycle_manager", executable="lifecycle_manager",
             name="lifecycle_manager_map",
             parameters=[{"node_names": ["map_server"], "use_sim_time": True}]),
        # C3: AMCL 定位 (替代真机的 small_point_lio + localization)
        Node(package="nav2_amcl", executable="amcl",
             name="amcl",
             parameters=[{"use_sim_time": True,
                         "base_frame_id": "base_link",
                         "odom_frame_id": "odom",
                         "global_frame_id": "map", "transform_tolerance": 0.2}]),
        Node(package="nav2_lifecycle_manager", executable="lifecycle_manager",
             name="lifecycle_manager_localization",
             parameters=[{"node_names": ["amcl"], "use_sim_time": True}]),
    ])
