...
| SSH push on port 22 closed during key exchange; no `gh` or HTTPS credential helper is installed | 19 | use GitHub's official SSH endpoint on port 443 with the existing SSH key |
| approval service rejected a second amend after its review stream disconnected | 19 | avoid history rewriting; preserve the feature commit and create a separate publication-log commit |
