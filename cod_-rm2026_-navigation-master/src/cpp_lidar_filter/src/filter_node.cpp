...
public:
  LidarFilterNode() : Node("lidar_filter_node")
  {
    ...
    this->declare_parameter("negative", true); // true = cut out the middle (robot body)

    std::string input_topic = this->get_parameter("input_topic").as_string();
    std::string output_topic = this->get_parameter("output_topic").as_string();

    RCLCPP_INFO(this->get_logger(), "Listening on (PointCloud2): %s", input_topic.c_str());
    RCLCPP_INFO(this->get_logger(), "Publishing to (PointCloud2): %s", output_topic.c_str());
...
private:
  ...
  {
    // PCL CropBox processing (removes robot body from point cloud)
