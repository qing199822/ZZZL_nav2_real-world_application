...
4. [模块详解](#4-模块详解)
   ...
   - 4.11 [串口通信](#411-serial_def_sdk--ros2_simple_serial--串口通信)
...
| 辅助传感器 | Intel RealSense D435i 深度相机 |
...
**作用：** 创造 `base_link_fake` —— 一个**始终朝向"正前方"**的虚拟机器人坐标系。

这是本项目最容易被误解但极其重要的模块。

#### 问题

Nav2 默认机器人前进方向是 X 轴正方向（航向角为 0）。但在 RoboMaster 比赛中，机器人经常**小陀螺旋转**（车身自旋巡检）。如果 Nav2 用真实朝向，速度指令会随旋转剧烈抖动，底盘无法正确执行。

#### 解决方案

创建一个 `base_link_fake` 帧，通过施加反向旋转"抹平"真实航向：
...
// 步骤2: Nav2 输出的是 "base_link_fake 系" 的速度 (X=向前)
//        需要转回 真实 base_link 系的速度
void FakeVelTransform::cmdVelCallback(const Twist::SharedPtr msg) {
    double angle = current_robot_base_angle_;

    Twist aft_tf_vel;
    // 2D 旋转变换: 把 "前方" 的速度旋转到真实机体朝向
    aft_tf_vel.linear.x = msg->linear.x * cos(angle) + msg->linear.y * sin(angle);
    aft_tf_vel.linear.y = -msg->linear.x * sin(angle) + msg->linear.y * cos(angle);

    cmd_vel_chassis_pub_->publish(aft_tf_vel);  // → /aft_cmd_vel
}
```

**通俗比喻：** Nav2 永远认为"正北是前进"，你的同事负责画地图也按正北画。`fake_vel_transform` 就像翻译——你说"向北走"，他看你当前的朝向，翻译成"向西北走"。

---

### 4.4 Nav2 导航核心栈

由 `navigation_launch.py` 启动，包含 7 个生命周期节点和一个生命周期管理器：
...
# 速度平滑器接收 cmd_vel_nav, 输出到 cmd_vel
remappings=[('cmd_vel', 'cmd_vel_nav'), ('cmd_vel_smoothed', 'cmd_vel')]
```

数据流：`controller_server → /cmd_vel_nav → velocity_smoother → /cmd_vel → fake_vel_transform → /aft_cmd_vel → 串口`
...
**解决方案：** 在 MPPI 外面包一层 wrapper，根据距离应用不同策略：
...
geometry_msgs::msg::TwistStamped computeVelocityCommands(...) {
    ...
    if (dist < direct_approach_distance_) {  // < 2.0m: 直接驱动模式
        // ★ 完全绕过 MPPI, 用 P 控制器朝目标直线走
...
|-----------|------|------|
| > 2.5m | 纯 MPPI | 正常运行 |
| 2.0m ~ 2.5m | MPPI + 限速 ≤ 0.2m/s | 降速接近 |
| < 2.0m | P 控制器直线驱动 | 完全绕过 MPPI |
...
2. **stvl_voxel_layer**（Spatio-Temporal Voxel Layer）— 来自 LiDAR + 深度相机的实时 3D 障碍物，带时间衰减
...
realsense_source:
  ...
  obstacle_range: 3.0                # 相机有效距离比 LiDAR 短
  raytrace_range: 3.5
  min_obstacle_height: 0.18
  max_obstacle_height: 1.5
  model_type: 0                      # 0 = 深度相机 (锥形投影)
  vertical_fov_angle: 1.01           # D435i 垂直 FOV ~58°
  horizontal_fov_angle: 1.52         # D435i 水平 FOV ~87°
```

**双传感器融合的优势：** LiDAR 覆盖 360° 远距离，深度相机补充近距离低矮障碍物（比如小台阶、线缆），两者通过 `combination_method: 1`（取最大值）叠加。
...
### 4.11 `serial_def_sdk` / `ros2_simple_serial` — 串口通信
...
```
/aft_cmd_vel (ROS 消息)  →  serial_def_sdk  →  /dev/ttyACM0  →  STM32  →  电机
...
```
┌──────┬──────────┬──────────┬──────────┬──────────┐
│ 0xA5 │  vx (f32)│  vy (f32)│  vz (f32)│  crc16   │
│ 1B   │  4B      │  4B      │  4B      │  2B      │
└──────┴──────────┴──────────┴──────────┴──────────┘
 总计 15 bytes / 帧
```
...
```cpp
// cod_serial.cpp — 打包逻辑
void topic_callback(const Twist::SharedPtr msg) {
    uint8_t packet[15];
    packet[0] = 0xA5;                            // 帧头 (魔数, 用于同步)
    std::memcpy(&packet[1], &msg->linear.x, 4);  // float → 4 bytes (小端)
    std::memcpy(&packet[5], &msg->linear.y, 4);
    std::memcpy(&packet[9], &msg->linear.z, 4);

    // CRC16 校验: 检测数据传输错误
    uint16_t checksum = 0xA5;
    for (int i = 1; i < 13; ++i) checksum += packet[i];
    std::memcpy(&packet[13], &checksum, 2);

    uart.writeBuffer(packet, 15);
}
```

#### Seasky 协议（`serial_def_sdk`）

更完善的串口通信模块，使用 Seasky 通信协议（带 CRC 校验、超时检测、双向通信），除了发送速度指令还可以接收底盘状态反馈。
...
```
/livox/lidar (3D 点云, ~20000 点/帧) → pointcloud_to_laserscan → /scan (2D, ~360 点/帧)
```
...
```
map ──→ odom ──→ base_link ──→ base_link_fake (Nav2 使用的帧)
  ↑        ↑         │
  │        │         ├──→ livox_frame (雷达安装位置)
  │        │         └──→ camera_link (RealSense 相机)
  │        │
  │        └── small_point_lio 发布 (~100Hz)
  │             激光里程计实时计算
  │
  └── 静态 TF (单点模式) 或被 slam_toolbox 动态更新 (SLAM 模式)
```
...
| 变换 | 发布者 | 频率 | 含义 |
|------|--------|------|------|
| `map → odom` | 静态 TF (或 slam_toolbox) | 静态/动态 | 地图到里程计的偏移 |
| `odom → base_link` | small_point_lio | ~100Hz | 机器人的实时位姿 |
| `base_link → base_link_fake` | fake_vel_transform | 随 /Odometry | 去旋转后的虚拟坐标系 |
| `base_link → livox_frame` | 用户静态 TF | 静态 | 雷达在车上的安装位 |
| `base_link → camera_link` | realsense TF | 来自驱动 | 相机在车上的安装位 |

### 5.3 为什么 Nav2 使用 `base_link_fake`

Nav2 需要一个"稳定"的机器人朝向来正确计算路径跟踪。如果使用真实 `base_link`，机器人在小陀螺旋转时，朝向持续变化，控制器输出的速度指令方向会混乱。

`base_link_fake` 通过反向旋转变换，使其 X 轴始终指向机器人**初始前方**，为 Nav2 提供一个稳定的参考帧。
...
| `base_link` | `base_link_fake` | 全向底盘小陀螺时朝向不稳定 |
