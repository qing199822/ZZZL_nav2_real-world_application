/**
 * @file    crc16.c
 * @brief   CRC-16-IBM 校验实现 (编译期静态表, 线程安全)
 *
 * 与上位机 serial_def_sdk/uart/crc16.c 完全一致。
 * 查表在编译期完成, 无运行时初始化, 多上下文安全。
 */

#include "crc16.h"

/* CRC-16-IBM 查找表 (多项式 0xA001), 编译期常量, 位于 .rodata / flash */
...
uint16_t crc_16(const uint8_t *input_str, uint16_t num_bytes)
{
    ...
    if (ptr != NULL) {
        for (a = 0; a < num_bytes; a++) {
            crc = (crc >> 8) ^ crc_tab16[(crc ^ (uint16_t)*ptr++) & 0x00FFU];
        }
    }
    return crc;
}
